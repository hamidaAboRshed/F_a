    ...
    */
    ...
    protected function init()
    {
        if (get_cookie('userID')) {
            $this->userID = get_cookie('userID');
            $this->userName = get_cookie('userName');
            $this->roles =unserialize(get_cookie('roles')) ;
    ...
    public function login($request)
    {
        ...
        if ($this->validate($request)) {
            ...
            if ($this->user) {
                ...
                $this->CI->session->set_userdata('login_error', 'Invalid login credentials');  
    ...
    protected function credentials($username, $password)
    {
        ...
        if($row && strcmp(md5($password),$row['Password'])==0) {
            return $row;
        }
    ...
    protected function setUser()
    {
        ...
        $employee = $this->CI->db->get_where('employee',['ID'=>$this->user['EmployeeID']])->row(0);
        $full_name = '';
        $first_name='';
        $position='';
        if($employee != null)
        {
            ...
            {
                $position_row = $this->CI->db->get_where('position',['id',$employee->PositionID])->row(0);
                $position = $position_row->Name;
            }
        }

        set_cookie('roles',serialize($this->userWiseRoles()),'10000000');
        set_cookie('userID',$this->user['ID'],'10000000');
        set_cookie('username',$this->user['Username'],'10000000');
        set_cookie('loginStatus',true,'10000000');
        set_cookie('Full_name',$full_name,'10000000');
        set_cookie('emp_id',$employee->ID,'10000000');
        set_cookie('Position',$position,'10000000');
        set_cookie('user_id',$this->user['ID'],'10000000');
        set_cookie('firstname',$this->$first_name,'10000000');
    ...
    public function userWiseRoles()
    {
       return array_map(function ($item) {
            return $item["role_id"];
        }, $this->CI->db->get_where("user_role", array("user_id" => $this->userID()))->result_array());
    }
    ...
    public function userRoles()
    {
        return array_map(function ($item) {
            ...
            ->select("role.*")
            ->from("role")
            ->join("user_role", "role.id = user_role.role_id", "inner")
            ->where(array("user_role.user_id" => $this->userID(),"role.status" => 1, "deleted_at" => null))
            ->get()->result_array());
    }
    ...
    public function userPermissions()
    {
        ...
        ->select("permission.*")
        ->from("permission")
        ->join("role_permission", "permission.id = role_permission.permission_id", "inner")
        ->where_in("role_permission.role_id", $this->roles())
        ->where(array("permission.status" => 1, "deleted_at" => null))
